...
class MultiplicationExeCategoryScreen extends StatefulWidget {
  final int type;

  const MultiplicationExeCategoryScreen({Key key, @required this.type})
      : super(key: key);

  @override
  _MultiplicationExeCategoryScreenState createState() =>
      _MultiplicationExeCategoryScreenState();
}

class _MultiplicationExeCategoryScreenState
    extends State<MultiplicationExeCategoryScreen> {
  int selectedIndex = 0;
  @override
              ...
              GestureDetector(
                onTap: () {
                  switch (selectedIndex) {
                    case 0:
                      Get.toNamed(Routers.multiplicationChoiceAnsEx,
                          arguments: {'type': widget.type});
                      break;
                    case 2:
                      Get.toNamed(Routers.multiplicationFillAnswerEx,
                          arguments: {'type': widget.type});
                      break;
                    case 1:
                      Get.toNamed(Routers.multiplicationDragImageEx,
                          arguments: {'type': widget.type});
                      break;
                    case 3:
                      Get.toNamed(Routers.multiplicationChoiceParagraphEx,
                          arguments: {'type': widget.type});
                      break;
                    default:
                  }
